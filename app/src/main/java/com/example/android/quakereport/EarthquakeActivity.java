...
//        earthquakeData.add(new EarthquakeData("1.6","Paris", "Oct 30, 2012"));
        ...
        final EarthquakeListAdaptor listAdaptor = new EarthquakeListAdaptor(this, QueryUtils.extractEarthquakes());

        // Set the adapter on the {@link ListView}
        // so the list can be populated in the user interface
        assert earthquakeListView != null;
        earthquakeListView.setAdapter(listAdaptor);
        earthquakeListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                EarthquakeData earthquakeData = listAdaptor.getItem(i);
